...
export const AddRecord = () => {
  ...
  const handleSearchVerify = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      // Auto-format UPH- prefix if missing
  ...
  const handleUploadRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ type: 'success', text: 'Record uploaded successfully to patient timeline!' });
        setTimeout(() => {
          setStep(1);
          setUpharId('');
          setPatientData(null);
        ...
        setMessage({ type: 'error', text: 'Failed to upload record.' });
      }
    } catch (error) {
